...
class Stats { 	
	...
	function currentworkers() {
		...
		$sql = "select count(a.username) from (select distinct username from shares where time > DATE_SUB(now(), INTERVAL 60 MINUTE) UNION select distinct username from shares_history where time > DATE_SUB(now(), INTERVAL 60 MINUTE)) a";
